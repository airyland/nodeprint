...
class Weibo {
	 ...
	 */
	function __construct($client_id='1605330496', $client_secret='899fadd78687700f72c3ca4e1b83deea', $access_token = NULL, $refresh_token = NULL) {
	...
	function requestAuthorizeCode(){
		header("location:".$this->getAuthorizeURL('http://nodeprint.com/oauth/weibo/callback'));
	}

	function setAuthorizeCode($code){
	...
	function requestAccessToken() {
		...
		/*if ( $type === 'token' ) {
			...
			$params['redirect_uri'] = 'http://nodeprint.com/oauth/weibo/callback';
	    ...
	    public function parse_user_info(){
	    	$c=new SaeTClientV2($this->client_id,$this->client_secret,$this->access_token);
       $this->user_info = $user_info = $c->show_user_by_id($this->openid);
       print_r($user_info);
        $user_data = array(
            'user_from'=>'weibo',
